import { Backpack, BookOpen, Gem, GitMerge, Map as MapIcon, Newspaper, Scroll, Shield, Sword, Users } from 'lucide-react';
    ...
    <aside className="basis-[22%] min-w-0 flex flex-col gap-4 h-full overflow-y-auto pl-3 custom-scrollbar column-scroll">
      ...
      <Panel title="行动">
        <div className="grid grid-cols-2 gap-2">
            <GoldButton onClick={() => onOpenModal('INVENTORY')} icon={<Backpack size={16} />}>背包</GoldButton>
            <GoldButton onClick={() => onOpenModal('SKILLS')} icon={<BookOpen size={16} />}>技能</GoldButton>
            <GoldButton onClick={() => onOpenModal('QUESTS')} icon={<Scroll size={16} />}>任务</GoldButton>
            <GoldButton onClick={() => onOpenModal('SOCIAL')} icon={<Users size={16} />}>社交</GoldButton>
            <div className="col-span-2">
               <GoldButton onClick={() => onOpenModal('PARALLEL_EVENTS')} icon={<GitMerge size={16} />}>并行事件条目</GoldButton>
            </div>
            <div className="col-span-2 mt-2 border-t border-stone-800 pt-2">
               <GoldButton onClick={() => onOpenModal('MAP')} icon={<MapIcon size={16} />} className="border-gold-500 text-gold-300">世界地图</GoldButton>
            </div>
        </div>
      </Panel>
...
const EquipmentRow: React.FC<{ label: string; item: EquipmentItem | null; icon: React.ReactNode }> = ({ label, item, icon }) => {
  return (
