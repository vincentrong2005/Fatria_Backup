...
const MapView: React.FC<{ showGrid: boolean }> = ({ showGrid }) => {
  ...
  return (
    <div
      className="w-full h-full flex items-center justify-center bg-black overflow-hidden relative group"
    ...
    >
      ...
      {showGrid &&
        locations.map((loc, index) => {
          ...
          const shouldShowTooltip = isClicked; // 点击时显示
          ...
          return (
            ...
            >
              ...
              <div className={`relative w-3 h-3 transition-all ${isClicked ? 'scale-125' : ''}`}>
                ...
                <div
                  className={`absolute inset-0 rounded-full bg-[var(--gold-500)] ${isClicked ? 'ring-2 ring-[var(--gold-400)]' : ''}`}
                />
              </div>

              {/* 悬浮信息卡（hover时显示） */}
              <div
                className={`${tooltipClasses} opacity-0 group-hover/point:opacity-100 ${isClicked ? 'hidden' : ''}`}
                style={style}
                onClick={e => e.stopPropagation()}
              >
