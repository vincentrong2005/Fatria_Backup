...
const App: React.FC = () => {
  ...
  const apps = [
    ...
    { id: 'INVENTORY' as ModalType, name: '背包', icon: 'Backpack', color: 'linear-gradient(135deg, #f59e0b, #fbbf24)' },
    { id: 'QUESTS' as ModalType, name: '任务', icon: 'Quest', color: 'linear-gradient(135deg, #10b981, #34d399)' },
    { id: 'RELATIONSHIPS' as ModalType, name: '关系', icon: 'Relationship', color: 'linear-gradient(135deg, #ec4899, #f472b6)' },
  ...
  const renderAppContent = () => {
    switch (activeApp) {
      case 'CHARACTER': return <CharacterApp state={gameState} />;
      case 'INVENTORY': return <div className="mt-4"><InventoryContent state={gameState} /></div>;
      case 'QUESTS': return <EmptyPlaceholder text="任务列表" />;
      case 'RELATIONSHIPS': return <EmptyPlaceholder text="关系网络" />;
      case 'COMBAT': return <EmptyPlaceholder text="战斗系统" />;
      case 'MAP': return <EmptyPlaceholder text="校园地图" />;
      case 'SETTINGS': return <EmptyPlaceholder text="系统设置" />;
      default: return null;
    }
  };
