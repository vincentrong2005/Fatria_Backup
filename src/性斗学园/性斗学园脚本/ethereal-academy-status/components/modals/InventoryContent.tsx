      ...
      <div className="md:col-span-2">
         ...
         <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {items.map(([name, item]: [string, InventoryItem]) => (
              <GlassCard key={name} interactive className="p-3 bg-white/5 aspect-square flex flex-col justify-between group">
                <div className="flex justify-between items-start">
                   <span className={`text-[10px] px-1.5 py-0.5 rounded border ${
                     item.等级 === 'S' || item.等级 === 'SS' ? 'border-yellow-500/50 text-yellow-300 bg-yellow-500/10' : 
                     item.等级 === 'A' ? 'border-purple-500/50 text-purple-300 bg-purple-500/10' :
                ...
                <div className="text-center my-2">
                  <div className="font-medium text-sm text-white/90 group-hover:text-brand-primary transition-colors">{name}</div>
                </div>
                <div className="text-[10px] text-white/40 line-clamp-2 leading-tight">
                  {item.描述}
                </div>
              </GlassCard>
            ))}
            ...
            {Array.from({ length: Math.max(0, 12 - items.length) }).map((_, i) => (
               <div key={`empty-${i}`} className="rounded-2xl border border-white/5 bg-black/10 aspect-square flex items-center justify-center">
