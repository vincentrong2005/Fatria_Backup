...
const StatsPanel: React.FC<StatsPanelProps> = ({ stats, align = 'left', compact = false }) => {
  const StatItem = ({ icon, label, value, color }: { icon: React.ReactNode, label: string, value: number, color: string }) => (
    <div className={`flex items-center justify-between ${compact ? 'p-1' : 'p-2'} rounded-lg ${compact ? '' : 'bg-white/5 border border-white/5'} ${align === 'right' && !compact ? 'flex-row-reverse' : ''}`}>
  ...
  return (
    <div className={`grid grid-cols-2 gap-x-2 gap-y-1 w-full ${compact ? 'max-w-[180px]' : 'max-w-xs'} animate-fade-in`}>
        <StatItem icon={<Zap size={compact ? 12 : 14} />} label="性斗力" value={stats.sexPower} color="text-pink-400" />
        <StatItem icon={<Shield size={compact ? 12 : 14} />} label="忍耐力" value={stats.baseEndurance} color="text-green-400" />
        <StatItem icon={<Wind size={compact ? 12 : 14} />} label="闪避率" value={stats.evasion} color="text-cyan-400" />
        <StatItem icon={<Crosshair size={compact ? 12 : 14} />} label="暴击率" value={stats.crit} color="text-orange-400" />
        ...
        <div className="col-span-2">
            <StatItem icon={<Brain size={compact ? 12 : 14} />} label="意志力" value={stats.willpower} color="text-purple-400" />
        </div>
    </div>
  );
};
