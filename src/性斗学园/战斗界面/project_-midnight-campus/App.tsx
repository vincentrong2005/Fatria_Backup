...
import { Sword, Backpack, Flag, HeartPulse, Brain, Zap, Activity, Terminal, FastForward, MessageCircle, Clock } from 'lucide-react';
    ...
    setTimeout(() => {
      setEnemy(currentEnemy => {
         setPlayer(currentPlayer => {
            ...
            const skillToUse = nextEnemyChar.skills.find((s: Skill) => s.id === turnState.enemyIntention?.id) || nextEnemyChar.skills[0];
            ...
            if (nextPlayerChar.stats.currentPleasure < nextPlayerChar.stats.maxPleasure) {
                 setTimeout(() => {
                    ...
                    setPlayer(p => {
                       ...
                       regenPlayer.stats.currentEndurance = Math.min(regenPlayer.stats.maxEndurance, regenPlayer.stats.currentEndurance + 5);
  ...
  const renderCharacter = (char: Character, isEnemy: boolean) => (
    <div className="flex-1 min-w-0 flex flex-col items-center relative max-w-[45%] lg:max-w-md transition-all duration-300">
      ...
      <div className="relative group w-full aspect-square max-w-[160px] lg:max-w-[280px] mb-2 lg:mb-6 cursor-help">
        <div className={`absolute inset-0 rounded-2xl blur-xl lg:blur-3xl opacity-20 group-hover:opacity-40 transition-opacity duration-700 ${isEnemy ? 'bg-rose-600' : 'bg-cyan-600'}`}></div>
        ...
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 p-2 rounded-2xl bg-black/60 backdrop-blur-sm border border-white/10">
           <div className="text-[10px] lg:text-xs text-white/70 uppercase tracking-widest mb-1 lg:mb-2 font-bold">详细属性</div>
                    ...
                    <div key={activeMenu} className="animate-slide-up h-full">
                      {activeMenu === 'main' && (
                          <div className="flex gap-2 h-full w-full">
                              <Card hover onClick={() => setActiveMenu('skills')} className="flex-1 min-w-0 flex flex-col items-center justify-center gap-2 group border-neonblue/20">
                              ...
                              <Card hover onClick={() => setActiveMenu('items')} className="flex-1 min-w-0 flex flex-col items-center justify-center gap-2 group">
                              ...
                              <Card hover onClick={() => addLog('不能逃跑！这是尊严之战！', 'system')} className="flex-1 min-w-0 flex flex-col items-center justify-center gap-2 group border-red-500/20">
                      ...
                      {activeMenu === 'skills' && (
                          <div className="flex gap-2 h-full w-full overflow-x-auto no-scrollbar pb-1">
                              {player.skills.map(skill => {
                                  ...
                                  return (
                                  <Card key={skill.id} hover={!isDisabled} onClick={() => !isDisabled && handlePlayerSkill(skill)} className={`flex-1 min-w-[100px] flex flex-col justify-between group relative overflow-hidden ${isDisabled ? 'opacity-60 grayscale-[0.5] border-white/5 bg-slate-900/50' : ''}`}>
                                       {isCooldown && (
                                      ...
                                      <div>
                                          <div className="flex justify-between items-start mb-1">
                                              <span className={`font-bold truncate mr-1 text-sm ${isDisabled ? 'text-slate-400' : 'text-neonblue'}`}>{skill.name}</span>
                                              <span className={`text-[10px] font-mono px-1 rounded whitespace-nowrap ${canAfford ? 'bg-white/10 text-slate-300' : 'bg-red-500/20 text-red-300 border border-red-500/30'}`}>{skill.cost} SP</span>
                                          </div>
                                          <p className="text-[10px] text-slate-400 leading-relaxed line-clamp-2">{skill.description}</p>
                                      </div>
                                      <div className={`mt-2 text-[10px] uppercase font-bold transition-colors ${isDisabled ? 'text-slate-600' : 'text-slate-600 group-hover:text-neonblue'}`}>
                                          {skill.type}
                                      </div>
                                  </Card>
                                  );
                              })}
                              <button onClick={() => setActiveMenu('main')} className="bg-white/5 hover:bg-white/10 rounded-xl flex items-center justify-center text-sm font-bold text-slate-400 min-w-[60px]">
                                  返回
                              </button>
                          </div>
                      )}
                      ...
                      {activeMenu === 'items' && (
                          <div className="flex gap-2 h-full w-full overflow-x-auto no-scrollbar pb-1">
                              {player.items.map(item => (
                                  <Card key={item.id} hover onClick={() => handlePlayerItem(item)} className={`flex-1 min-w-[100px] flex flex-col justify-between group ${item.quantity === 0 ? 'opacity-50 grayscale pointer-events-none' : ''}`}>
                                      <div>
                                          <div className="flex justify-between items-start mb-1">
                                              <span className="font-bold text-emerald-400 truncate mr-1 text-sm">{item.name}</span>
                                              <span className="text-[10px] font-mono bg-emerald-900/40 border border-emerald-500/30 px-2 py-0.5 rounded text-emerald-200 whitespace-nowrap">x{item.quantity}</span>
                                          </div>
        ...
        {(turnState.phase === 'victory' || turnState.phase === 'defeat') && (
            <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xl flex items-center justify-center animate-fade-in">
                <div className="text-center">
                    <h2 className={`text-6xl md:text-8xl font-black mb-4 tracking-tighter ${turnState.phase === 'victory' ? 'text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-600' : 'text-slate-600'}`}>
