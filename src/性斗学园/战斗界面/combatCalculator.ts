...
export function applyDefenseReduction(baseDamage: number, targetEndurance: number): number {
  ...
  console.info(
    `[防御减伤] 减伤公式: ${baseDamage} * 50 / (${targetEndurance} + 100) = ${baseDamage} * 50 / ${denominator}`,
  );
  console.info(
    `[防御减伤] 计算过程: ${baseDamage} * 50 = ${baseDamage * 50}, ${baseDamage * 50} / ${denominator} = ${finalDamage}`,
  );
...
export function executeAttack(attacker: Character, target: Character, skill: SkillData): CombatResult {
  ...
  logs.push(
    `防御减伤公式: ${damageBeforeDefense} × 50 ÷ (${targetEndurance} + 100) = ${damageBeforeDefense} × 50 ÷ ${targetEndurance + 100}`,
  );
  logs.push(
    `计算过程: ${damageBeforeDefense} × 50 = ${damageBeforeDefense * 50}, ${damageBeforeDefense * 50} ÷ ${targetEndurance + 100} = ${Math.floor((damageBeforeDefense * 50) / (targetEndurance + 100))}`,
  );
