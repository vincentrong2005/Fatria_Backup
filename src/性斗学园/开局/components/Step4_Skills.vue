      ...
      <div class="p-4 h-[350px] overflow-y-auto custom-scrollbar">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <button
            ...
            :disabled="!data.initialPassiveSkills.includes(constitution.id) && data.initialPassiveSkills.length >= MAX_PASSIVE_SKILLS"
            :class="[
              'relative group p-3 rounded-xl border transition-all duration-300 text-left flex flex-col gap-2',
          ...
          >
            ...
            <div class="flex items-start gap-3">
              <div :class="[
                'p-2.5 rounded-lg transition-colors shrink-0',
                data.initialPassiveSkills.includes(constitution.id) ? 'bg-secondary text-white' : 'bg-black/30 text-gray-400 group-hover:text-gray-200'
              ]">
            ...
            <div v-if="constitution.permanentModifiers.length > 0" class="flex flex-wrap gap-1.5">
              <span
                ...
                :class="[
                  ...
                  mod.value > 0 ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'
                ]"
              >
                {{ getModifierLabel(mod.stat) }} {{ mod.value > 0 ? '+' : '' }}{{ mod.value }}{{ mod.isPercent ? '%' : '' }}
              </span>
            </div>
            ...
            <div v-if="constitution.sensitivityModifiers.length > 0" class="flex flex-wrap gap-1.5">
              ...
              >
                {{ sens.bodyPart === 'all' ? '全身' : sens.bodyPart }}敏感度 {{ sens.modifier > 0 ? '+' : '' }}{{ sens.modifier }}%
              </span>
            </div>
