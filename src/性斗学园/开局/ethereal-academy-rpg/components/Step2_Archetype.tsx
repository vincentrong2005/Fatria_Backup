    ...
    <div className="animate-slide-up space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-black/20 border border-white/5 rounded-t-2xl p-4 flex justify-between items-center backdrop-blur-md">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-secondary/20 rounded text-secondary"><User size={18} /></div>
              <div>
              ...
              {currentArchetypes.map((archetype: Archetype) => {
                ...
                return (
                  <button
                    ...
                    className={`relative group p-3 rounded-xl border transition-all duration-300 text-left flex items-start gap-3 ${isSelected
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <h4 className={`font-bold text-sm truncate transition-colors ${isSelected ? 'text-white' : 'text-gray-200'}`}>
                          {archetype.name}
                        </h4>
                      </div>
        ...
        <div className="lg:col-span-1">
          <div className={`h-full rounded-2xl border border-white/10 p-6 flex flex-col items-center text-center transition-all duration-500 ${selectedArchetype ? 'bg-gradient-to-b from-secondary/10 to-transparent' : 'bg-white/5 grayscale opacity-50'}`}>
            {selectedArchetype ? (
              <>
                ...
                <div className="mt-6 w-full pt-6 border-t border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {Object.entries(selectedArchetype.baseBonus).map(([key, value]) => {
                      ...
                      return (
                        <span key={key} className="text-xs px-2 py-1 rounded-md bg-black/40 text-gray-200 border border-white/10">
                          {getStatLabel(key)} <span className={isPositive ? "text-green-400" : "text-red-400"}>{isPositive ? '+' : ''}{numVal}</span>
                        </span>
                      );
                    })}
                  </div>
          ...
          <div className="mb-8 bg-black/20 p-4 rounded-xl border border-white/5">
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer group">
                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${data.configFeatures.hasBreasts ? 'bg-secondary border-secondary' : 'border-gray-500 bg-transparent'}`}>
                  {data.configFeatures.hasBreasts && <Icons.Check size={14} className="text-white" />}
                </div>
                <input type="checkbox" className="hidden" checked={data.configFeatures.hasBreasts} onChange={() => toggleFeature('hasBreasts')} />
                <span className={`text-sm group-hover:text-white transition-colors ${data.configFeatures.hasBreasts ? 'text-white' : 'text-gray-400'}`}>女性性征 (乳房/臀部)</span>
              </label>

              <label className="flex items-center gap-2 cursor-pointer group">
                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${data.configFeatures.hasPenis ? 'bg-secondary border-secondary' : 'border-gray-500 bg-transparent'}`}>
                  {data.configFeatures.hasPenis && <Icons.Check size={14} className="text-white" />}
                </div>
                <input type="checkbox" className="hidden" checked={data.configFeatures.hasPenis} onChange={() => toggleFeature('hasPenis')} />
                <span className={`text-sm group-hover:text-white transition-colors ${data.configFeatures.hasPenis ? 'text-white' : 'text-gray-400'}`}>男性性征 (阴茎)</span>
              </label>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            <label className="flex justify-between text-sm font-medium text-gray-300 mb-4">
              <span className="flex items-center gap-2"><Ruler size={16} /> 身高 (Height)</span>
          ...
          {showBreasts && (
            <div>
              <label className="flex justify-between text-sm font-medium text-gray-300 mb-4">
                <span className="flex items-center gap-2"><Circle size={16} /> 臀围 (Hips)</span>
          ...
          {showPenis && (
            <div>
              <label className="flex justify-between text-sm font-medium text-gray-300 mb-4">
                <span className="flex items-center gap-2"><Zap size={16} /> 阴茎长度 (Length)</span>
              ...
              <div className="flex flex-wrap gap-2">
                {CUP_SIZES.map((size) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg text-sm font-medium transition-all duration-200 ${data.cupSize === size
            ...
            <div className="flex flex-wrap gap-2">
              {currentGenitalOptions.map((type: string) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 border ${data.genitalType === type
