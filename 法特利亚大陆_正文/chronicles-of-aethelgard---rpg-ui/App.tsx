...
function extractContentFromRaw(raw: string): { content: string; options: string[] } {
  ...
  // 第四步：使用用户给定正则语义: 捕捉 <content>...</content> 中间的内容
  // 注意：这里只用于“正文展示”，不能破坏原始 raw 中的 <image> 等标签，保证文生图功能正常
  const contentMatch = processed.match(/[\s\S]*?<content>([\s\S]*?)<\/content>/i);
  const contentSource = contentMatch && contentMatch[1] ? contentMatch[1] : processed;
  // 在用于显示的正文中，移除 <image>...</image> 区块，但不影响原始 raw
  const content = contentSource.replace(/<image>[\s\S]*?<\/image>/gi, '').trim();
